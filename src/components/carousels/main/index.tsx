...
import useEmblaCarousel, { EmblaOptionsType, EmblaCarouselType, EmblaEventType } from "embla-carousel-react"
...
const CarouselContext = createContext<StyleProps & { 
...
} & Partial<StyleProps>> = props => {

    const { children, options, variant="default", size="default" } = props
    const [container, instance] = useEmblaCarousel(options)
...
export const Slide: React.FC<{
    ...
    return <Box __css={style.slide} {...rest}>{children}</Box>
}
...
export const Counter: React.FC<{} & BoxProps> = props => {
    ...
    return <Box {...rest} color="#fff">{currentIndex} / {context.instance.slideNodes().length}</Box>
}